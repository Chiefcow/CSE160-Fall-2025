...
class TestSim:
    ...
    def cmdTestServer(self, address, port):
        ...
        print "Test Server command sent to node", address, "port", port

    def cmdTestClient(self, source, dest, srcPort, destPort, transfer):
        """
        Initiates the client at [source] and connects to [dest]:[destPort].
        Sends [transfer] bytes of data.
        """
        # Pack parameters: dest(1), srcPort(1), destPort(1), transfer(2)
        ...
        print "Test Client command sent from node", source, "to", dest

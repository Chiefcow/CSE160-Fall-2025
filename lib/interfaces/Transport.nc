...
//     command error_t bind(socket_t fd, socket_addr_t *addr);
//     command error_t listen(socket_t fd);
//     command socket_t accept(socket_t fd);  // Changed from event
//     command error_t connect(socket_t fd, socket_addr_t * addr);
//     command error_t close(socket_t fd);
//     command error_t release(socket_t fd);  // Added
//     command uint16_t write(socket_t fd, uint8_t *buff, uint16_t bufflen);  // Changed from send
//     command uint16_t read(socket_t fd, uint8_t *buff, uint16_t bufflen);   // Added
//     command error_t receive(pack* msg);
